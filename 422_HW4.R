...
## 2.2 (b): Dimensions of characteristics matrix:- Number of unique shingles(rows)  * Number of files(columns) = 17614*100
...
data_full <- read.csv("u.csv", sep="\t", header=T, comment.char = "#")
data_item <- read.csv("u_item.csv", sep="|", header=T, comment.char = "#")
...
#cluster_similarity(movie127[,6:24], genre50, similarity="jaccard", method="independence")
